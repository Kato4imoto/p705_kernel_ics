...
#if 0		// samjinjang I2C recover QCT patch 0514
...
//static void qup_i2c_recover_bus_busy(struct qup_i2c_dev *dev)		// samjinjang I2C recover QCT patch 0514
...
//	if (dev->pdata->msm_i2c_config_gpio)		// samjinjang 0507 remove
//	{
//		dev_err(dev->dev, "dev->pdata->msm_i2c_config_gpio's\n");
//		return;
//	}
	if (!(status & (I2C_STATUS_BUS_ACTIVE)) ||
		(status & (I2C_STATUS_BUS_MASTER)))
	{
		dev_err(dev->dev, "status & I2C_STATUS_BUS_MASTER\n");
		return;
	}
	...
	dev_err(dev->dev, "Bus still busy, status %x\n", status);
			...
			if (!timeout) {
				...
				if (!(istatus & I2C_STATUS_BUS_ACTIVE) ||
					...
					wait_for_completion_timeout(&complete,
									HZ);
#if 0		// I2C recover QCT patch 0514
					if (timeout)
						goto timeout_err;
#else
					if (timeout)
...
#else		// samjinjang I2C recover QCT patch 0514
timeout_err:
			if (dev->err) {
				if (dev->err > 0 &&
					dev->err & QUP_I2C_NACK_FLAG) {
