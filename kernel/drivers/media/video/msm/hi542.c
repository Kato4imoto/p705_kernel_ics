...
static int32_t hi542_i2c_write_b_sensor(unsigned short waddr, uint8_t bdata)
{
	...
	for (loop = 0; loop < 3; loop++)
	{
		...
		/* LGE_CHANGED_S (20120502), donghyunkwon@lge.com : add comment hynix sw_reset error message */
		if((waddr == 0x0001) && (bdata == 0x02) && rc < 0) {
			pr_err("hynix SW reset called message: addr = 0x%x, val = 0x%x\n", waddr, bdata);
			break;
		}
		/* LGE_CHANGED_E (20120502), donghyunkwon@lge.com : add comment hynix sw_reset error message */
...
static void hi542_start_stream(void)
{
	...
	/* LGE_CHANGED_S (20120502), donghyunkwon@lge.com : patch #5 for camframe timeout issue */
...
//  msleep(20);
	/* LGE_CHANGED_E (20120502), donghyunkwon@lge.com : patch #5 for camframe timeout issue */
	CDBG("%s: %d  Exit \n",__func__, __LINE__);
}
...
//	 int16_t target_dist, small_step, next_lens_position;		 //Remove this part if you use slew mode.
	...
	/* LGE_CHANGED_S (20120502), donghyunkwon@lge.com : patch for vcm noise when vcm power off */
	hw_damping = 0xF;
	hi542_sw_damping_time_wait = 4500;// //4.5 msec
	/* LGE_CHANGED_E (20120502), donghyunkwon@lge.com : patch for vcm noise when vcm power off */
...
/*LGE_CHANGE_S : Hi542 sensor stabilization
  ...
  apply X-shutdown mode,sensor reset stabilization*/
	...
	/* LGE_CHANGED_S (20120502), donghyunkwon@lge.com : patch #5 for camframe timeout issue */
	hi542_i2c_write_b_sensor(0x03d4, 0x18);//20120416 ryu 18);/* LDO level */20120418 ryu 28 20120612 QE request to change from 0x28 to 0x18
	msleep(100);	//20120509 ryu add

/* LGE_CHANGE_E : Hi542 sensor stabilization */	
     
		rc = hi542_i2c_write_b_table(hi542_regs.rec_settings,
				hi542_regs.rec_size);
if (rc < 0)		// samjinjang 0517 retry i2c init fail
		{	
			pr_err("REG_INIT failed \n");
			goto sensor_setting_fail;
		}
	} 
...
/* LGE_CHANGE_E : Hi542 sensor stabilization */	
		...
		{		
			if (!CSI_CONFIG) 
			{
				//msleep(50); //20120509 ryu delete
				msleep(20); //20120509 ryu add
			...
			rc =hi542_i2c_write_b_table(hi542_regs.reg_prev,
		...
		struct msm_sensor_ctrl *s)
{
	...
	s->s_stop = hi542_sensor_stop;		// QCT patch sensor stop stream
