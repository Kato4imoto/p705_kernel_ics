...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			if (sync->sctrl.s_stop)		// QCT patch sensor stop stream
				sync->sctrl.s_stop();
			...
			int is_controlnode)
{
	...
	int retry_loop; /* LGE_CHANGE : Add retry 		// samjinjang 0517 retry i2c init fail */
	...
	if (!sync->core_powered_on && !is_controlnode) {
		...
		if (sync->vfefn.vfe_init) {
			...
			sync->get_pic_abort = 0;
#if 0		// samjinjang 0517 retry i2c init fail
...
#else
			for (retry_loop = 0; retry_loop < 3; retry_loop++) {
				rc = msm_camio_sensor_clk_on(sync->pdev);
				if (rc < 0) {
					pr_err("%s: setting sensor clocks failed: %d\n",
						__func__, rc);
